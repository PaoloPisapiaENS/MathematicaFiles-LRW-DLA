...
Options[ToContinuum]={"print"->False,"justSmallestOrder"->True,"\[Gamma]gterms"->False,"\[Kappa]Order"->6};
...
\[Gamma]Action=\[CapitalMu]Action-freeAction-gAction/.{\[Kappa]^5->0,\[Kappa]^6->0}//Expand;
...
(* \[Gamma]g action *)
\[Gamma]gAction = Expand[ReplaceAll[\[CapitalMu]Action - freeAction - gAction, {\[Kappa]^3 -> 0, \[Kappa]^4 -> 0, \[Gamma]^2 -> 0}]]; \[Gamma]gAction = ReplaceAll[\[Gamma]gAction, {\[Chi]s[x, t, 1, 1] Derivative[2, 0][\[Phi]s][x, t] :> DD[\[Chi]s[x, t, 1, 1] \[Phi]s[x, t]] - \[Phi]s[x, t] Derivative[2, 0, 0, 0][\[Chi]s][x, t, 1, 1] - 2 Derivative[1, 0][\[Phi]s][x, t] Derivative[1, 0, 0, 0][\[Chi]s][x, t, 1, 1]}]; \[Gamma]gAction = Collect[\[Gamma]gAction, {\[Gamma] \[Phi][x, t], \[Delta]x \[Delta]x, \[Chi][x, t, Subscript[i, 1], Subscript[\[Alpha], 1]] \[Chi]s[x, t, Subscript[i, 1], Subscript[\[Alpha], 1]]}, FullSimplify]; If[OptionValue["print"], Print[Style["\t \[Gamma]g-interaction action \!\(\*SubscriptBox[\(S\), \(\(\[Gamma]g\)\( \)\)]\)= ", {Blue}], g \[Gamma]gAction, "\n"]]; Print[Style["\t \[Gamma]g-interaction action after purification with ", {Blue, Bold}], Style[{\[Delta]t -> 0}, {Blue}], Style[" \!\(\*SubscriptBox[\(S\), \(\(\[Gamma]g\)\( \)\)]\)-> ", {Blue, Bold}], ReplaceAll[g \[Gamma]gAction, {\[Delta]t -> 0}], "\n"]; Null
];
