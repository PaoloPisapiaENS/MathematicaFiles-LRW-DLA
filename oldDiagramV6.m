...
FreeAction[time_]:=V[1+Sum[Product[ \[Psi]s[Subscript[x, ToExpression["action"<>ToString[ time]<>ToString[ index1]]],Subscript[t, time+1]]\[Psi][Subscript[x, ToExpression["action"<>ToString[ time]<>ToString[ index1]]],Subscript[t, time]],{index1,1,index2}],{index2,1,time}]
];
...
V\[Psi]\[Chi][space_,time_]:= V[-\!\(\*OverscriptBox[\(g\), \(~\)]\) \[Psi][space,Subscript[t, time]]\[Chi][space,Subscript[i, 1],Subscript[\[Alpha], 1],Subscript[t, time]]\[Chi]s[space,Subscript[i, 1],Subscript[\[Alpha], 1],Subscript[t, time]]];
...
V[\[Psi]s[x,Subscript[t, time+1]]\[Psi][x,Subscript[t, time]]]+V\[Gamma]1[x,time]+V\[Gamma]2[x,time]+V\[Gamma]2Minus[x,time]+V\[Psi]\[Chi][x,time]
];


(* ::Input::Initialization:: *)
ClearAll[WickContractionSpaceVertices];
Options[WickContractionSpaceVertices]={"print"->False,"fields"->FieldsNamesGenerator[{\[Phi],\[Chi],\[Psi]}], "endTime"->1,"#vertices"->10,"\[Delta]rule"->True,"\[Delta]ruleExternal"->True,"\!\(\*SubscriptBox[\(n\), \(\[Chi]\)]\)"->(-1),"\!\(\*SubscriptBox[\(n\), \(\[Alpha]\)]\)"->(0),"kill\[Psi]"->True};

WickContractionSpaceVertices[A_ +B_,OptionsPattern[]]:=WickContractionSpaceVertices[A,"print"->OptionValue["print"],"fields"->OptionValue["fields"], "endTime"->OptionValue["endTime"],"#vertices"->OptionValue["#vertices"],"\[Delta]rule"->OptionValue["\[Delta]rule"],"\[Delta]ruleExternal"->OptionValue["\[Delta]ruleExternal"],"\!\(\*SubscriptBox[\(n\), \(\[Chi]\)]\)"->OptionValue["\!\(\*SubscriptBox[\(n\), \(\[Chi]\)]\)"],"\!\(\*SubscriptBox[\(n\), \(\[Alpha]\)]\)"->OptionValue["\!\(\*SubscriptBox[\(n\), \(\[Alpha]\)]\)"],"kill\[Psi]"->OptionValue["kill\[Psi]"]]+
...
If[OptionValue["kill\[Psi]"],z=z/.\[Psi]s[___,Subscript[t, a_]]/;(a<OptionValue["endTime"]+1):>0];
...
(* TRUE *)
factors=List@@expr;
chis=Select[factors,!FreeQ[#,\!\(\*OverscriptBox[\(g\), \(~\)]\)]&&!FreeQ[#,Subscript[t, time]]&];
...
Options[WickContractionSpaceVerticesMOD]={"print"->False,"fields"->FieldsNamesGenerator[{\[Phi],\[Chi],\[Psi]}], "endTime"->1,"#vertices"->10,"\[Delta]rule"->True,"\[Delta]ruleExternal"->True,"\!\(\*SubscriptBox[\(n\), \(\[Chi]\)]\)"->(-1),"\!\(\*SubscriptBox[\(n\), \(\[Alpha]\)]\)"->(0),"maxContractions"->5,"kill\[Psi]"->True};
...
WickContractionSpaceVerticesMOD[V[A_] *B_, OptionsPattern[]]:=Block[{locFields={}, starFields={}, deltaFields={},deltaStarFields={},contributions=0,z,zTemp1,zTemp2,\[Kappa],jj,l,timeIndex,ii,ss},

z=V[A]*B;

(* NEW BIT *)
If[!FreeQ[z,\!\(\*OverscriptBox[\(g\), \(~\)]\)],
(* TRUE *)


If[OptionValue["print"],Print["TRUE: z before =", z]];
z=z//.V\[Psi]\[Chi][Subscript[x, a_],t]:>(Sum[V\[Psi]\[Chi][Subscript[x, a],timeIndex],{timeIndex,(a-100),OptionValue["endTime"]}])//Expand;(* a must be given as 100+(Natural Numbers)*)
...
(*zTemp1=Replace[z,{ locFields[[l]][a___,Subscript[t, timeIndex]]:>locFields[[l]][a,Subscript[t, timeIndex]]+\[Kappa] *deltaFields[[l]][a,Subscript[t, timeIndex]]},{1,4}];*)
...
zTemp2=zTemp1/. starFields[[l]][a___,Subscript[t, timeIndex]]:>starFields[[l]][a,Subscript[t, timeIndex]]+\[Kappa] *deltaStarFields[[l]][a,Subscript[t, timeIndex]];
(*zTemp2=Replace[zTemp1,{ starFields[[l]][a___,Subscript[t, timeIndex]]:>starFields[[l]][a,Subscript[t, timeIndex]]+\[Kappa] *deltaStarFields[[l]][a,Subscript[t, timeIndex]]},{1,4}];*)
...
zTemp1=Expand[zTemp1];
zTemp2=zTemp1//.deltaFields[[l]][x_,a___,Subscript[t, timeIndex]]*deltaStarFields[[l]][y_,b___,Subscript[t, timeIndex]](*/;(!NumericQ[a*b]):>*):>Subscript[G, locFields[[l]]][x,y,Subscript[t, timeIndex]]*\[Delta][{a},{b}]; (*G is the propagator*)
(*zTemp2=Replace[zTemp1,{ deltaFields[[l]][x_,a___,Subscript[t, timeIndex]]*deltaStarFields[[l]][y_,b___,Subscript[t, timeIndex]](*/;(!NumericQ[a*b]):>*):>Subscript[G, locFields[[l]]][x,y,Subscript[t, timeIndex]]*\[Delta][{a},{b}]},{1,2}];*)
...
z=z/.Subscript[G, \[Psi]][Subscript[x, action],Subscript[x, action],__]->0/.Subscript[G, \[Phi]][a_,a_,b_] ->0/.Subscript[G, \[Chi]][a_,a_,b_] ->0;

];(*End of loop over time*)

If[OptionValue["kill\[Psi]"],
z=z/.\[Psi]s[___,Subscript[t, a_]]/;(a<=OptionValue["endTime"]):>0
];
...
(*Subscript[F_, \[Chi]][Subscript[x, a_],Subscript[x, b_],Subscript[t, c_]]Subscript[F_, \[Chi]][Subscript[x, d_],Subscript[x, e_],Subscript[t, c_]]:>{Style[FindArc[{b,c},{a,c},1][[1]],FindArc[{b,c},{a,c},1][[2]],Darker[Green]],Style[FindArc[{e,c},{d,c},4][[1]],FindArc[{e,c},{d,c},4][[2]],Darker[Green]]},*)
(* THIS IS FOR TADPOLES*)
Subscript[F_, \[Chi]][Subscript[x, a_],Subscript[x, a_],Subscript[t, c_]]:>Style[FindArc[{a,c},{a+0.01,c},1][[1]],FindArc[{a,c},{a+0.01,c},1][[2]],Darker[Green]],
Subscript[F_, \[Chi]][Subscript[x, a_],Subscript[x, b_],Subscript[t, c_]]:>Style[FindArc[{b,c},{a,c},(b-a)][[1]],FindArc[{b,c},{a,c},(b-a)][[2]],Darker[Green]],


Subscript[F_, \[Phi]]^n_[Subscript[x, a_],Subscript[x, b_],Subscript[t, c_]]:>Style[FindArc[{b,c-1},{a,c},n][[1]],FindArc[{b,c-1},{a,c},n][[2]],Red],
Subscript[F_, \[Phi]][Subscript[x, a_],Subscript[x, 0],Subscript[t, 1]]:>Style[Line[{{a-0.13,1-0.13},{a,1}}],Arrowheads[{0,.03}],Red],
...
pureProp=DeleteElements[terms[[i]],{-1,g,\!\(\*OverscriptBox[\(g\), \(~\)]\),Subscript[g, \[Gamma]],\[Gamma],\[Gamma]1,\[Gamma]2,-g,-\!\(\*OverscriptBox[\(g\), \(~\)]\),-Subscript[g, \[Gamma]],-\[Gamma],-\[Gamma]1,-\[Gamma]2,g^n_?NumericQ,\!\(\*OverscriptBox[\(g\), \(~\)]\)^n_?NumericQ,Subscript[g, \[Gamma]]^n_?NumericQ,\[Gamma]^n_?NumericQ,\[Gamma]1^n_?NumericQ,\[Gamma]2^n_?NumericQ}];
pureProp=DeleteCases[pureProp,_?NumericQ];
...
pureProp=pureProp/.{Subscript[F_, \[Chi]]^n_[Subscript[x, a_],Subscript[x, b_],Subscript[t, c_]]:>Style[FindArc[{b,c},{a,c},n][[1]],FindArc[{b,c},{a,c},n][[2]],Darker[Green]],
Subscript[F_, \[Chi]][Subscript[x, a_],Subscript[x, a_],Subscript[t, c_]]:>Style[FindArc[{a+0.5,c},{a,c},1][[1]],FindArc[{a+0.5,c},{a,c},1][[2]],Darker[Green]],
Subscript[F_, \[Chi]][Subscript[x, a_],Subscript[x, b_],Subscript[t, c_]]:>Style[FindArc[{b,c},{a,c},(b-a)][[1]],FindArc[{b,c},{a,c},(b-a)][[2]],Darker[Green]],
