...
Clear[Vertex\[Gamma]2,Vertex\[Gamma]2Minus,Vertex\[Gamma]g,Vertex\[Gamma]2gMinus,Vertex\[Psi]\[Psi],Vertex\[Psi]\[Chi],Vertex\[Chi]\[Chi]];
Vertex\[Gamma]2[index_]:= Module[{},
Return[{{\[Gamma] \[Phi][p,Subscript[kk, index]]},{k^2 \[Chi]s[k,Subscript[i, index]]\[Phi]s[q,Subscript[kk, index+1]]DiracDelta[p+q+k]\[Delta][Subscript[kk, index],Subscript[kk, index+1]]}}];
]

Vertex\[Gamma]2Minus={{-\[Gamma] \[Phi][p,1]},{(k+q)^2 \[Chi]s[k,Subscript[i, 1]]\[Phi]s[q,2]\[Psi]s[s,3]DiracDelta[p+q+k+s]\[Delta][1,2]}};

Vertex\[Gamma]g={{-\[Gamma] g \[Phi][p,1]\[Chi][k1,Subscript[i, 2]]},{k^2 \[Chi]s[k,Subscript[i, 1]]\[Phi]s[q,2]\[Chi]s[k2,Subscript[i, 3]]DiracDelta[p+q+k+k1+k2]\[Delta][Subscript[i, 2],Subscript[i, 3]]}};
Vertex\[Gamma]2gMinus={{\[Gamma] g \[Phi][p,1]\[Chi][k1,Subscript[i, 2]]},{(k+q)^2 \[Chi]s[k,Subscript[i, 1]]\[Phi]s[q,2]\[Psi]s[s,3]\[Chi]s[k2,Subscript[i, 3]]DiracDelta[p+q+k+s+k1+k2]\[Delta][1,2]\[Delta][Subscript[i, 2],Subscript[i, 3]]}};

Vertex\[Psi]\[Psi]={{-g \[Psi][p,1]\[Psi][k,2]},{\[Psi]s[q,3]\[Psi]s[s,4]DiracDelta[p+q+k+s]\[Delta][1,3]\[Delta][2,4]}};
Vertex\[Psi]\[Chi]={{-g \[Psi][p,1]\[Chi][k,Subscript[i, 2]]},{\[Psi]s[q,3]\[Chi]s[s,Subscript[i, 4]]DiracDelta[p+q+k+s]\[Delta][1,3]\[Delta][Subscript[i, 2],Subscript[i, 4]]}};
Vertex\[Chi]\[Chi]={{-g \[Chi][p,Subscript[i, 1]]\[Chi][k,Subscript[i, 2]]},{\[Chi]s[q,Subscript[i, 3]]\[Chi]s[s,Subscript[i, 4]]DiracDelta[p+q+k+s]\[Delta][Subscript[i, 1],Subscript[i, 3]]\[Delta][Subscript[i, 2],Subscript[i, 4]]}};


(* ::Input::Initialization:: *)
ClearAll[WickContractionMomentum];
(*Options[WickContractionMomentum]={"print"->False,"fields"->FieldsNamesGenerator[{\[Phi],\[Psi],\[Chi]}], "endTime"->1,"#vertices"->10};*)

WickContractionMomentum[{{A__},{B__}}]:=WickContractionMomentum[{{A},{B}}]=WickContractionMomentum[{A},{B}];
WickContractionMomentum[{A___}*{B___} ]:=WickContractionMomentum[Inner[Times,A,B]];

WickContractionMomentum[A_?NumericQ]:=A

(*Definition for each type of field*)
fieldsNames=FieldsNamesGenerator[{\[Phi],\[Psi],\[Chi]}];
Fields=fieldsNames[[1]];
starFields=fieldsNames[[2]];

Module[{l,ll},
For[l=1,l<=Length[Fields],l++,

For[ll=1,ll<=Length[starFields],ll++,

If[ll==l,
WickContractionMomentum[{A_*Fields[[l]][p_,a_]},{B_*starFields[[ll]][q_,b_]}]:=WickContractionMomentum[{A},{B}]\[Delta][a,b]DiracDelta[p+q]int[p],
(*ELSE*)
WickContractionMomentum[{Fields[[l]][p_,a_]},{starFields[[ll]][q_,b_]}]:=Fields[[l]][p,a]starFields[[ll]][q,b]
]
];(*End of loop on ll*)

WickContractionMomentum[{A_*Fields[[l]][p_,a_]},{B_}]:= Fields[[l]][p,a]*B* A;

If[l<=Length[starFields],
WickContractionMomentum[{A_},{B_*starFields[[l]][q_,b_]}]:=starFields[[l]][q,b]*B* A;
]
];(*End of loop on l*)
]

(*
WickContractionMomentum[{a__},{b__}]:= Sum[ WickContractionMomentum[{List[a][[1]] },{List[b][[j]]}] * WickContractionMomentum[Flatten@{List[a][[2;;]]},Flatten@{List[b][[;;j-1]],List[b][[j+1;;]]}],{j,1,Length[List[b]]} ];*)
